// INPUT ACTIONS
export const ENTER_CHARACTER = 'ENTER_CHARACTER';
export const KEY_LEFT = 'KEY_LEFT';
export const KEY_RIGHT = 'KEY_RIGHT';
export const BACKSPACE = 'BACKSPACE';
export const ENTER = 'ENTER';

// MODULE PROGRESS ACTIONS
export const MOVE_TO_NEXT_STAGE = 'MOVE_TO_NEXT_STAGE';
export const MOVE_TO_PREVIOUS_STAGE = 'MOVE_TO_PREVIOUS_STAGE';
export const SUBMIT_ANSWER = 'SUBMIT_ANSWER';
export const COMPLETE_MODULE = 'COMPLETE_MODULE';
export const RETURN_TO_ALL_MODULES = 'RETURN_TO_ALL_MODULES';
export const START_MODULE = 'START_MODULE';
export const MARK_STAGE_AS_COMPLETE = 'MARK_STAGE_AS_COMPLETE';

export const CONTINUE_MODULE = 'CONTINUE_MODULE';
export const START_OVER_MODULE = 'START_OVER_MODULE';
